...
## Cleanup Phase Notes (Don’t Forget Later)

These are consistent sources of visual drift today. We should address them after all archetypes are locked and story-backed.

- Questions overriding label typography/color via `labelClassName` (example: `text-black/70`).
  - Goal: labels should be owned by primitives (`FormLabel` / `FieldInput` / `FieldSelect`) and use tokens.
- Placeholder misuse:
  - Units in placeholders (units must be `unitAdornment` suffix).
  - Placeholders used as labels (placeholders should be optional examples only).
- Layout hacks inside questions:
  - Width/padding hacks like `md:w-2/3`, `md:px-20`, and `space-x-*` on grids.
  - Goal: archetype wrappers own layout; fields are `w-full`.
- Label alignment hacks:
  - `label="&nbsp;"` used to align inputs/selects.
  - Goal: add an explicit primitive affordance (e.g. `labelHidden`/`labelSpacer`) so questions don’t invent markup.
- Token bypass:
  - Hard-coded colors like `text-black/70`, `text-black/60` should be replaced by token classes (`text-foreground/*`, `text-secondary`, etc.).
