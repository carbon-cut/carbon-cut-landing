...
- project metadata
- reference to the current `Inventory`

It does not carry draft/calculated/outdated lifecycle state.

### `Inventory`

`Inventory` is one saved inventory revision for a project.
...
- the inventory has saved `setup` and/or `inventory`
...
- the inventory is no longer the active current revision
...
- a project can have many `Inventory` revisions
...
- resuming restores the current active `Inventory` for the project
- save-for-later does not require calculation

## Calculation runs

- one project can have many `CalculationRun`
- each run belongs to one project and one `Inventory`
- run history is kept
- a successful run provides the `CalculationResult` for that inventory revision

## Latest result

- a `calculated` inventory points to its latest successful run
- a current `draft` inventory may have no current result yet
- results from older inventory revisions remain history

## Storage rules

- `Project` stores workspace identity and metadata
- `Project` points to the current active `Inventory`
- `Inventory` stores `setup`, `inventory`, lifecycle status, and `lockedYears`
- historical `Inventory` revisions are stored when the workflow needs them
